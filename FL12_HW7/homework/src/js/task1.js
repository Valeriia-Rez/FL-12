let promptMessagePassword, promptMessageEmail, confirmMessage, promptMessageOldPassword, promptMessageNewPassword, promptMessageNewAgainPassword;
...
if (promptMessageEmail === "" || promptMessageEmail === null) {
    alert(alertCancel);
} else if (promptMessageEmail.length < 5) {
    ...
    if (promptMessagePassword === "" || promptMessagePassword === null) {
        alert(alertCancel);
    } else if (promptMessageEmail === userEmail && promptMessagePassword === userPassword || promptMessageEmail === adminEmail && promptMessagePassword === adminPassword) {
        alert(alertCorrect);
        confirmMessage = confirm("Do you want to change your password?");
        if (confirmMessage) {
            promptMessageOldPassword = prompt("Enter your old password");
            if (promptMessageOldPassword === "" || promptMessageOldPassword === null) {
                alert(alertCancel);
            } else if (promptMessageEmail === userEmail && promptMessageOldPassword === userPassword || promptMessageEmail === adminEmail && promptMessageOldPassword === adminPassword) {
                alert(alertCorrect);
                promptMessageNewPassword = prompt("Enter new password");
                if (promptMessageNewPassword === "" || promptMessageNewPassword === null) {
                    alert(alertCancel);
                } else if (promptMessageNewPassword.length < 6) {
